...
void* cpu_thread(void* arg) {
    ...
    // Cálculo intensivo: aproximação de π pela série de Leibniz
    long n = 100000000; // mais iterações para consumir CPU
    double pi = 0.0;
    int sign = 1;
    for (long i = 0; i < n; i++) {
        pi += sign * 4.0 / (2 * i + 1);
    ...
    printf("Thread CPU-bound %ld terminou (resultado: %.10f)\n", (long)arg, pi);
    return NULL;
}
